...
void Timer2_Init(void)
{
	...
	g_Timer2Virtual[0].Timer_Status = TIMER_START;
	g_Timer2Virtual[0].Timer_Set = 2*100;	// 10ms   2s
	...
	rTCFG1 = (rTCFG1 & ~(0xf<<8)) | (0x2<<8); //divider:8，0b0010
	
    rTCNTB2 = 33000;  //让定时器2每隔1秒中断一次 33000=1*33000
	rTCMPB2 = 0;
	
	
	//rTCNTB2 = 37500;    		// Timer input clock Frequency = PCLK / {prescaler value+1} / {divider value}	100 Hz -- 10ms
	//rTCMPB2 = 20000;
Timer2_Stop();
...
void __irq timer3_ISR(void)
{
	int i;
	timeadd++;
	/////////////////
	if(timeadd >= g_InteralMemory.KeepWord[13])///设定时间g_InteralMemory.KeepWord[13]到达停止送经
	{
		Set_Y_Value(11,0);	
		
		if((g_InteralMemory.KeepWord[13] >  g_InteralMemory.KeepWord[270]) || (g_InteralMemory.KeepBit[620] == 0))
		{
			timeadd=0;
			rINTMSK1 =  rINTMSK1 | ((U32)0x1<<13);
		}
		
		
	}
	
	if((timeadd >=g_InteralMemory.KeepWord[270]) && (g_InteralMemory.KeepBit[620] > 0 ))/////KW13 < KW270
	{
		Set_Y_Value(5,1);//反转
			
		Pulse_Out_Init();
		PulseOut_1_Start(600,g_InteralMemory.KeepWord[271]);
		
		Delaytime=0;
		Timer2_Stop();
		
		
		if(g_InteralMemory.KeepWord[270] >= g_InteralMemory.KeepWord[13])
		{
			timeadd=0;
			rINTMSK1 =  rINTMSK1 | ((U32)0x1<<13);
		}
...
{	
	//g_InteralMemory.KeepWord[56]=1;
	
	rSRCPND1 = rSRCPND1 | ((U32)0x1<<13);   //清空定时器3源请求
    rINTPND1 = rINTPND1 | ((U32)0x1<<13);    //清空定时器3中断请求
    rINTMSK1 =  rINTMSK1 & ~((U32)0x1<<13);//打开定时器3中断    
    //定时器配制寄存器3 
    //定时器输入时钟频率 = PCLK / {预分频值+1} / {分频值}
    //{预分频值} = 0~255 {分频值} = 2, 4, 8, 16 
   
    rTCFG0 = (rTCFG0 & ~(0xff<<8)) | (249<<8); // prescaler1:249
 	rTCFG1 = (rTCFG1 & ~(0xf<<12)) | (0x2<<12); //divider:8，0b0010
	
    rTCNTB3 = 33000;  //让定时器3每隔1秒中断一次 33000=1*33000
	rTCMPB3 = 0;
	
	
    rTCON = (rTCON  | (11<<16)); //自动重载、手动更新、启动定时器3
    rTCON = (rTCON & ~(0x2<<16));    //关闭手动更新
