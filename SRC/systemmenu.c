...
void DisplayParScr()
{
	...
	Lcd_Display_String(Left,Top,6,0,9,2,TongXunDuanKou,0);  //通讯端口设定:
	Lcd_Fill_Box(162*2,94,214*2,(94+36),11);

	j=new_NO[0];
	length=strlen(Str[0][j]);
   	Lcd_DispAsc_On_Button(162*2,94,214*2,130,length,9,0,1,0,2,Str[0][j]);
	num[0]=3;       
	num[1]=26;                                                   //:
	...
	Lcd_Display_String(144,152,1,0,9,0,num,0);
		...
		{	
			...
			if(X_result>=224*2&&X_result<=313*2&&Y_result>=138.5*2&&Y_result<=2*168.5)   //button 确定
			{
				...
				Lcd_Button(Left+8, Top+10, 2*313, Top+70,5, 2,INSERT);
	            		Lcd_DispHZ_On_Button(Left+8, Top+10, 2*313, Top+70,2,0,5,2,0,0,0,3,QueDing); 
			...
			if(X_result>=273*2&&X_result<=2*313&&Y_result>=2*61&&Y_result<=2*86)   //button down  you
			{
				...
				Lcd_DispHZ_On_Button(2*273, Top,2*313, 2*86,1,0,5,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
			...
			if(X_result>=2*224&&X_result<=2*264&&Y_result>=2*61&&Y_result<=2*86)   //button up  zuo
			{
				...
				Lcd_DispHZ_On_Button(2*224, 2*61,2*264, 2*86,1,0,5,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
			...
			if(X_result>=2*224&&X_result<=2*264&&Y_result>=2*108.5&&Y_result<=2*133.5)   //button -
			{
				...
				Lcd_DispHZ_On_Button(2*224, Top+18*2, 2*264, Top+43*2,1,0,5,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
			...
			if(X_result>=2*273&&X_result<=2*313&&Y_result>=2*108.5&&Y_result<=2*133.5)   //button +
			{
				...
				Lcd_Button(2*273, Top+36, 2*313, Top+43*2,5, 2,INSERT);
			 	Lcd_DispHZ_On_Button(2*273, Top+36, 2*313, Top+43*2,1,0,5,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
...
void DrawBackColor(char flag,char old_i,char new_i)
{
	...
	if(flag==3)	// button  Up Down
	{
		if(new_i!=old_i)
		{
			...
			Lcd_DispAsc_On_Button(2*162,top1 ,2*214,bottom1,length,9,0,1,0,2,Str[old_i][j]);
		}
	}
...
void DisplayScrCheck(char CurrentSrcNum)
{
	...
	Lcd_DispHZ_On_Button(0,2*40,2*320,2*56,9,0,9,2,0,1,0,0,All);  //应用程序共有画面数:
	num[0]=3;
	num[1]=26;
	...
	Lcd_DispAsc_On_Button(120*2, 2*86, 2*200, 2*117,strlen(str),9,0,1,0,0,str);

	Lcd_Button(50*2, 2*87, 2*100, 2*117,5, 2,RAISE);
	num[0]=1;
	num[1]= 92;
	Lcd_DispHZ_On_Button(50*2, 2*87, 2*100, 2*117,1,0,9,1,0,0,0,0,num);    //显示 +

	
	Lcd_Button(220*2, 2*87, 2*270, 2*117,5, 2,RAISE);
	num[0]=1;
	num[1]= 93;
	Lcd_DispHZ_On_Button(220*2, 2*87, 2*270, 2*117,1,0,9,1,0,0,0,0,num);    //显示 -

	Lcd_Button(120*2, 2*124, 2*200, 2*158,5, 2,RAISE);
	Lcd_DispHZ_On_Button(120*2, 2*124, 2*200, 2*158,2,0,9,2,0,0,0,0,JianCha);    //检查

	Lcd_Button(120*2, 2*165, 2*200, 2*199,5, 2,RAISE);
	Lcd_DispHZ_On_Button(120*2, 2*165, 2*200, 2*199,2,0,9,2,0,0,0,0,FanHui);    //返回
        ...
        while(1)
	{
		...
		if(flag==1)
		{
			...
			if(X_result>=2*50&&X_result<=2*100&&Y_result>=2*87&&Y_result<=2*117)   //button +
			{
				...
				num[0]=1;
				num[1]= 92;
				Lcd_DispHZ_On_Button(2*50, 2*87, 2*100, 2*117,1,0,9,1,0,0,0,0,num);    //显示 +
			...
			if(X_result>=2*220&&X_result<=2*270&&Y_result>=2*87&&Y_result<=2*117)   //button -
			{
				...
				num[0]=1;
				num[1]= 93;
				Lcd_DispHZ_On_Button(2*220, 2*87, 2*270, 2*117,1,0,9,1,0,0,0,0,num);    //显示 -
			...
			if(X_result>=2*120&&X_result<=2*200&&Y_result>=2*124&&Y_result<=2*158)   //button 检查
			{
				...
				Lcd_DispHZ_On_Button(2*120, 2*124, 2*200, 2*158,2,0,9,2,0,0,0,0,JianCha);    //检查
			...
			if(X_result>=2*120&&X_result<=2*200&&Y_result>=2*165&&Y_result<=2*199)   //button 返回
			{
				...
				Lcd_DispHZ_On_Button(2*120,2*165, 2*200, 2*199,2,0,9,2,0,0,0,0,FanHui);    //返回
