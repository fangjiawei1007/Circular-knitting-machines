...
//	INT8U Min[]={55,78,48,1,54,21,3,26};
	char Min[]="MinNum:";
	char QuXiao[]={40,1,47,91};
	char QuXiao1[]="Cancle";
	char QueDing[]={40,23,22,8};
	char QueDing1[]="OK";
	...
	Lcd_Button(0+KEYBOARDOFFSET,0,350+KEYBOARDOFFSET,480,12,3,RAISE);  // Background

	Lcd_Fill_Box(3+KEYBOARDOFFSET,3,347+KEYBOARDOFFSET,40, 3);    //Title
//	Lcd_DispHZ_On_Button(151+KEYBOARDOFFSET,7,200+KEYBOARDOFFSET,40,3,13,3,1,0,1,0,0,Info);
	long2 = strlen(Info);
	Lcd_Disp_AscStr(Info,long2, 120+KEYBOARDOFFSET, 7, 2,2,16,13,3);

	Lcd_Button(40+KEYBOARDOFFSET,50,300+KEYBOARDOFFSET,95,13,2,INSERT);//Number Display area

//	Lcd_DispHZ_On_Button(20+KEYBOARDOFFSET,110,91+KEYBOARDOFFSET,150,4,19,12,0,0,1,0,0,Max);
	long2 = strlen(Max);
	Lcd_Disp_AscStr(Max,long2, 20+KEYBOARDOFFSET, 111, 1,1,8,19,12);

//	Lcd_DispHZ_On_Button(20+KEYBOARDOFFSET,132,91+KEYBOARDOFFSET,150,4,19,12,0,0,1,0,0,Min);
	long2 = strlen(Min);
	Lcd_Disp_AscStr(Min,long2, 20+KEYBOARDOFFSET, 133, 1,1,8,19,12);

	//Display the MaxValue and the MinValue here
	long1=strlen(MaxValueStr);
	Lcd_Disp_AscStr(MaxValueStr,long1, 86+KEYBOARDOFFSET, 112, 1,1,8,19,12);
	long1=strlen(MinValueStr);
	Lcd_Disp_AscStr(MinValueStr,long1, 86+KEYBOARDOFFSET, 134, 1,1,8,19,12);
	...
	Num[0]=3;
	Num[1]=17;
	Lcd_Button(Left,Top,Left+70,Top+65,7,3,RAISE);
	Lcd_DispHZ_On_Button(Left,Top,Left+70,Top+65,1,0,7,1,0,0,0,0,Num);
	...
	Num[1]=91;
	Lcd_Button(Left,Top,Left+70,Top+65,7,3,RAISE);
	Lcd_DispHZ_On_Button(Left,Top,Left+70,Top+65,1,0,7,4,0,0,0,0,Num);
	...
	Left = 93+KEYBOARDOFFSET;
	Top = 390;
	Lcd_Button(Left,Top,Left+111,Top+65,7,3,RAISE);
	if(g_InteralMemory.KeepWord[110]==0)
	{
		Lcd_DispHZ_On_Button(Left,Top,Left+111,Top+65,2,0,7,1,0,0,0,0,QuXiao);
	}
	else
	{
		long2 = strlen(QuXiao1);
		Lcd_Disp_AscStr(QuXiao1,long2, Left+30, Top+18, 1,2,8,0,7);
	}
	...
	if(g_InteralMemory.KeepWord[110]==0)
	{
		Lcd_DispHZ_On_Button(Left,Top,Left+111,Top+65,2,0,7,1,0,0,0,0,QueDing);
	}
	else
	{
		long2 = strlen(QueDing1);
		Lcd_Disp_AscStr(QueDing1,long2, Left+45, Top+18, 1,2,8,0,7);
	}
...
int KeyBoard(int Max,int Min,U8 DigitPos,U8 DigitNumb)
{
	...
	char MaxValueStr[10],MinValueStr[10];
	int a;
	double Result1;
	...
	{	
		...
		if(flag==1)
		{
			...
			if(X_result>=10+KEYBOARDOFFSET && X_result<=80+KEYBOARDOFFSET && Y_result>=165&&Y_result<=230)   //button 1
			{
				...
				Num[0]=3;
				Num[1]=17;
				Lcd_Button(Left,Top,Left+70,Top+65,7,3,INSERT);
				Lcd_DispHZ_On_Button(Left,Top,Left+70,Top+65,1,0,7,1,0,0,0,0,Num);
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
			 	Beep(0);
...
//++++++++++++++++++++++++++++++++++++++
				if(g_InteralMemory.Word[101]==1 || g_InteralMemory.Word[101]==7|| g_InteralMemory.Word[101]==8|| g_InteralMemory.Word[101]==11|| g_InteralMemory.Word[101]==14
					|| g_InteralMemory.Word[101]==62 || g_InteralMemory.Word[101]==63|| g_InteralMemory.Word[101]==64|| g_InteralMemory.Word[101]==70)
					Lcd_DispAsc_On_Button(40+KEYBOARDOFFSET,50,300+KEYBOARDOFFSET,95,strlen(Str_password),13,0,1,2,2,Str_password);
				else
					Lcd_DispAsc_On_Button(40+KEYBOARDOFFSET,50,300+KEYBOARDOFFSET,95,strlen(Str),13,0,1,2,2,Str);
...
//++++++++++++++++++++++++++++++++++++++
				...
				l_P=strlen(Str_password);
//++++++++++++++++++++++++++++++++++++++				
				Lcd_Fill_Box(42+KEYBOARDOFFSET,52,298+KEYBOARDOFFSET,93,13);
				if(g_InteralMemory.Word[101]==1 || g_InteralMemory.Word[101]==7|| g_InteralMemory.Word[101]==8|| g_InteralMemory.Word[101]==11|| g_InteralMemory.Word[101]==14
					|| g_InteralMemory.Word[101]==62 || g_InteralMemory.Word[101]==63|| g_InteralMemory.Word[101]==64|| g_InteralMemory.Word[101]==70)
					Lcd_DispAsc_On_Button(40+KEYBOARDOFFSET,50,300+KEYBOARDOFFSET,95,strlen(Str_password),13,0,1,2,2,Str_password);
				else
			...
			if(X_result>=93+KEYBOARDOFFSET && X_result<=204+KEYBOARDOFFSET && Y_result>=390&&Y_result<=455)  //button cancel
			{
				...
				Left = 93+KEYBOARDOFFSET;
				Top = 390;
				Lcd_Button(Left,Top,Left+111,Top+65,7,3,INSERT);
				if(g_InteralMemory.KeepWord[110]==0)
				{
					Lcd_DispHZ_On_Button(Left,Top,Left+111,Top+65,2,0,7,1,0,0,0,0,QuXiao);
				}
...
void Func_ShiftButton(int StarAddr)
{
	...
	else
	{
		...
		if((ScrNum!=CurrentScrNum) &&( (CurrentScrNum==1) || CurrentScrNum==7))
		{
			flag = 0;
			while((i!=650623)&&(((i & 0xffff)!=g_InteralMemory.KeepWord[50])||(((i >> 16)& 0xffff)!=g_InteralMemory.KeepWord[51])) && i!=-1) 
			i=KeyBoard(999999,0,0,0);
		}
		if(i!=-1)
